...
In-paper anchor: `paper/MOTIVIC_ARCHITECTURE_PREPRINT.md` (`MOT_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the declared motivic realization and filtration package across admissible correspondence transport through a multi-lane motivic super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`MOT_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_MOT`.
...
If near-failure extraction and no-Zeno spacing hold, then `MOT_G3` prevents hidden drift before rigidity is applied.
