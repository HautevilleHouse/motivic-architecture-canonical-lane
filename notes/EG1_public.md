...
## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of the declared motivic realization and filtration package across admissible correspondence transport through a multi-lane motivic super-architecture`.
...
`MOT_G1` closes when `kappa_realization` satisfies the response-floor requirement: projected realization response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_MOT`.
...
Payload: check that all quantities used by `kappa_realization` are defined on the projected sector named in the main preprint.
...
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_realization`, then `MOT_G1` supplies the projected/protected-core input to `M_MOT`.
